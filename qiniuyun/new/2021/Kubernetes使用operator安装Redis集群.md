...
description: 通过部署集群部署有状态的应用会简单很多文档：在上管理集群每个主节点及其从节点都由管理，为每个创建无头，并为所有节点创建服务。每个有状态集都使用来确保主节点和从节点分散在不同的节点上。同时，当操作员在每。。。。。。。
...
![](http://image.ownit.top/csdn/20210225160104777.png)
